var SerieComponent = (function () {
  ...
  // How about building a string and then assigning it to innerHTML when it's completely built (so you never inject broken html).
  // Firing broken html into the browser bit by bit is bound to cause problems, because the browser cannot store a broken dom tree.
  SerieComponent.prototype.appendSerie = function (serie) {
    var that = this;
    ...
    if(!serie.added) {
        ...
        addActionHTML.addEventListener('click', function(){ that.mySerieApi.addSerie(serie); }, false);
        ...
        decActionHTML.addEventListener('click', function(){ that.mySerieApi.decSerie(serie, function() {
            newSerie.querySelector("progress").value = serie.current_episode;
        }); }, false);
        ...
        incActionHTML.addEventListener('click', function(){ that.mySerieApi.incSerie(serie, function() {
            newSerie.querySelector("progress").value = serie.current_episode;
        }); }, false);
        ...
        delActionHTML.addEventListener('click', function(){ that.mySerieApi.deleteSerie(serie, newSerie); }, false);
